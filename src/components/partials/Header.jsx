...
function Header() {
  ...
  return (
    <header className="fixed top-0 w-full bg-white shadow-sm z-50">
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          <div className="relative group">
            ...
            <div className="absolute -left-8 top-full z-10 w-screen max-w-md overflow-hidden rounded-3xl bg-white shadow-lg ring-1 ring-gray-900/5 hidden group-hover:block">
              <div className="p-4">
                <NavItem title="Analytics" description="Get a better understanding of your traffic" />
                <NavItem title="Engagement" description="Speak directly to your customers" />
                <NavItem title="Security" description="Your customers’ data will be safe and secure" />
                <NavItem title="Integrations" description="Connect with third-party tools" />
                <NavItem title="Automations" description="Build strategic funnels that will convert" />
      ...
      <div className={`lg:hidden fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10 transform transition-transform ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
...
function NavItemMobile({ title }) {
  ...
  return (
    <div className="-mx-3">
      <button type="button" className="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50" aria-controls="disclosure-1" aria-expanded="false" onClick={toggleChildVisibility}>
        {title}
        <svg className={`h-5 w-5 flex-none text-gray-400 ${isChildVisible ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          <path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clipRule="evenodd" />
        </svg>
      </button>

      {isChildVisible && <div className="mt-2 space-y-2" id="disclosure-1">
